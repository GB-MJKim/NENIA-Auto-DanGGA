...
var CSVParser = {
            ...
            for (var i = 1; i < lines.length; i++) {
                ...
                try {
                    ...
                    if (productData) {
                        ...
                        // 페이지 번호 수집
                        var pageNum = parseInt(productData["페이지"], 10);
                        if (pageNum) {
                            pageNumbers[parseInt(pageNum, 10)] = true;
                        }
                    }
                } catch (rowError) {
                    continue;
    ...
    findByPageAndOrder: function(data, pageNum, orderNum) {
        for (var i = 0; i < data.length; i++) {
            if (data[i]["페이지"] === pageNum && data[i]["순서"] === orderNum) {
                return data[i];
            }
        }
        return null;
...
var FileSelector = {
    ...
    categorizeFiles: function(fileList, csvPageNumbers) {
        ...
        for (var i = 0; i < fileList.length; i++) {
            ...
            // 정수로 변환해서 비교
            if (parseInt(pageNum, 10) && csvPageNumbers[parseInt(pageNum, 10)]) {
